...
function App() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "messages"), orderBy("timestamp", "asc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const msgs = snapshot.docs.map((doc: DocumentData) => ({
        id: doc.id,
        ...(doc.data() as Message),
      }));
      setMessages(msgs);
  ...
  // Deleta mensagem própria
  const handleDelete = async (id: string, owner: string | undefined) => {
    if (owner !== username) return;
    if (window.confirm("Deseja realmente deletar esta mensagem?")) {
      await deleteDoc(docRef(db, "messages", id));
    }
  };
  function getAvatarByUsername(name: string) {
    return usersData.find((u) => u.username === name)?.avatar || "";
  }
            ...
            .map((m) =>
              m.system ? (
                ...
                <MessageItem
                  ...
                  avatar={getAvatarByUsername(m.user!)} // ← aqui
                />
              )
            )}
          <div ref={messagesEndRef} />
